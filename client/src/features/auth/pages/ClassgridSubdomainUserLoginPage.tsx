...
export function ClassgridSubdomainUserLoginPage({ preferredRole }: { preferredRole?: AuthUserRole }) {
    ...
    getAuthBranding({ authType: "institution", slug })
      ...
      .catch((err) => {
        ...
        if (err.message?.includes("404")) {
          window.location.href = "https://classgrid.in";
        } else {
      ...
      >
        ...
        <section className="relative flex flex-col bg-background dark:bg-[#0f0f0f] border-r border-border dark:border-white/[0.15] overflow-hidden px-10">
          ...
          <div className="relative z-10 mx-auto mt-6 w-full max-w-[360px]">
            <h2 className="text-[22px] font-bold text-foreground dark:text-[#ededed]">
              Contact Information
            </h2>

            <div className="mt-5 flex items-center gap-4">
              <Mail className="h-6 w-6 shrink-0 text-[#10b981]" />
              <p className="text-[14px] text-foreground dark:text-[#ededed]">support@classgrid.in</p>
            </div>

            <div className="my-4 h-px bg-border dark:bg-white/[0.14]" />

            <div className="flex items-start gap-4">
              <MapPin className="mt-0.5 h-6 w-6 shrink-0 text-[#f97316]" />
              <p className="text-[14px] leading-[22px] text-foreground dark:text-[#ededed]">
        ...
        <section className="flex h-full items-center justify-center bg-muted dark:bg-[#111111] px-6 overflow-hidden">
          ...
          {/* 9. Inner Login Box */}
          <div className="flex w-full max-w-[500px] flex-col justify-center rounded-[24px] border border-border dark:border-white/[0.15] bg-background dark:bg-[#0f0f0f] px-10 py-8 shadow-xl">
            ...
            {step === 1 ? (
              <form onSubmit={handleLogin} className="flex flex-col">
                ...
                <h1 className="mt-4 text-center text-[24px] font-bold text-foreground dark:text-[#ededed]">{branding.name}</h1>
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setActiveRole("student")} className={`flex h-[44px] items-center justify-center gap-2 rounded-[12px] border text-[14px] font-medium text-foreground dark:text-[#ededed] transition-colors ${activeRole === "student" ? "border-[#10b981] bg-[#10b981]/10" : "border-border dark:border-white/[0.14] bg-muted dark:bg-[#111111] hover:bg-zinc-200 dark:hover:bg-[#222222]"}`}>
                    <GraduationCap className="h-[18px] w-[18px] text-[#10b981]" />
                    Student
                  </button>
                  <button type="button" onClick={() => setActiveRole("teacher")} className={`flex h-[44px] items-center justify-center gap-2 rounded-[12px] border text-[14px] font-medium text-foreground dark:text-[#ededed] transition-colors ${activeRole === "teacher" ? "border-[#f97316] bg-[#f97316]/10" : "border-border dark:border-white/[0.14] bg-muted dark:bg-[#111111] hover:bg-zinc-200 dark:hover:bg-[#222222]"}`}>
                ...
                <div className="flex h-[46px] items-center gap-3 rounded-[12px] border border-border dark:border-white/[0.14] bg-background dark:bg-[#141414] px-4">
                  ...
                  <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full bg-transparent text-[14px] text-foreground dark:text-[#ededed] outline-none placeholder:text-muted-foreground dark:text-white/40" placeholder="Email / Student ID" />
                </div>
                ...
                <div className="mt-3 flex h-[46px] items-center gap-3 rounded-[12px] border border-border dark:border-white/[0.14] bg-background dark:bg-[#141414] px-4">
                  ...
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="current-password" className="w-full bg-transparent text-[14px] text-foreground dark:text-[#ededed] outline-none placeholder:text-muted-foreground dark:text-white/40" placeholder="Password" />
                ...
                <div className="mt-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[13px] text-foreground dark:text-[#ededed]">Remember me</span>
                  </div>
                  {showForgotPassword && (
                ...
                {/* 17. Sign In Button */}
                <button type="submit" disabled={isSubmitting || !email || !password} className="mt-4 h-[46px] w-full rounded-[14px] bg-[#10b981] text-[15px] font-bold text-white transition-colors hover:bg-[#059669] disabled:opacity-50 shadow-lg shadow-emerald-900/50">
                  {isSubmitting ? "Signing In..." : "Sign In"}
                </button>

                {/* 18. Terms Text */}
                <p className="mt-3 text-center text-[11px] text-muted-foreground dark:text-white/55">
              ...
              <form onSubmit={handleVerifyDevice} className="flex flex-col">
                <button type="button" onClick={() => setStep(1)} className="mb-4 inline-flex w-fit items-center gap-2 text-[13px] font-medium text-muted-foreground dark:text-white/60 transition-colors hover:text-emerald-400">
                  <ArrowLeft className="h-4 w-4" /> Back
                </button>
                <h2 className="text-center text-[20px] font-bold text-foreground dark:text-[#ededed]">Verify this device</h2>
                ...
                <div className="mt-6 flex h-[44px] items-center gap-3 rounded-[12px] border border-border dark:border-white/[0.14] bg-background dark:bg-[#141414] px-4">
                  <Lock className="h-[18px] w-[18px] shrink-0 text-muted-foreground dark:text-white/70" />
                  <input type="text" maxLength={6} inputMode="numeric" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} required className="w-full bg-transparent text-[14px] text-foreground dark:text-[#ededed] outline-none placeholder:text-muted-foreground dark:text-white/40 tracking-[0.25em]" placeholder="000000" />
                </div>
                ...
                <button type="button" onClick={handleResendOtp} disabled={isResendingOtp || otpCooldownSeconds > 0} className="mt-3 h-[46px] w-full rounded-[12px] border border-border dark:border-white/[0.14] bg-transparent text-[14px] font-medium text-foreground dark:text-[#ededed] transition-colors hover:bg-white/5 disabled:opacity-50">
