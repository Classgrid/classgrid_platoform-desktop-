...
import { clearAuthCallbackError, getAuthCallbackFeedback, getDefaultRoleForAudience, getPostLoginPath, isUserRole, readStoredAuthRole, saveStoredAuthRole } from "../auth-helpers";
...
export function TestFullScreenLoginPage({ preferredRole }: { preferredRole?: AuthUserRole }) {
  ...
  const [branding, setBranding] = useState<AuthBranding | null>({
    authType: "institution",
    name: "Example Institution",
    shortName: "Example",
    tagline: "Learning, connected.",
    siteTitle: "Example Institution Portal",
              ...
              {step === 1 ? (
                <form onSubmit={handleLogin} className="flex flex-col" aria-busy={isSubmitting}>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <label className="flex items-center gap-3" htmlFor="test-remember-me">
                      <input id="test-remember-me" name="rememberMe" type="checkbox" checked={rememberMe} onChange={(event) => setRememberMe(event.target.checked)} className="h-4 w-4 accent-[#10b981]" />
                ...
                <form onSubmit={handleVerifyDevice} className="flex flex-col" aria-busy={isSubmitting || isResendingOtp}>
                  ...
                  <div className="mt-5 flex h-[46px] items-center gap-3 rounded-[12px] border border-border dark:border-white/[0.14] bg-background dark:bg-[#141414] px-4 transition-colors focus-within:border-emerald-500/50">
                    <Lock className="h-[18px] w-[18px] shrink-0 text-muted-foreground dark:text-white/70" />
                    <input id="test-device-code" name="deviceCode" type="text" maxLength={6} inputMode="numeric" autoComplete="one-time-code" aria-label="Six-digit device verification code" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} required className="w-full bg-transparent text-[16px] text-foreground dark:text-white outline-none placeholder:text-muted-foreground dark:placeholder-white/40 tracking-[0.25em] focus:ring-0 border-none focus:border-transparent" style={{ boxShadow: 'none', border: 'none', outline: 'none' }} placeholder="000000" />
                  </div>
