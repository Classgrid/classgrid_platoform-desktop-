...
import { Input } from "@/components/classgrid/Input";
...
export function ResultsProcessingPage() {
  ...
  return (
    <div className="cg-page max-w-6xl mx-auto">
      <header className="cg-page__header flex justify-between items-end">
        <div>
...
function SchemesList({ schemes, onSelect }: { schemes: ResultScheme[]; onSelect: (s: ResultScheme) => void }) {
  ...
  return (
    <div className="space-y-4">
      {schemes.map((scheme) => (
        <div key={scheme.id} className="cg-card p-4 flex items-center justify-between cursor-pointer hover:border-primary/50 transition-colors" onClick={() => onSelect(scheme)}>
          <div>
...
function GenerationEngine({ scheme }: { scheme: ResultScheme | null }) {
  ...
  return (
    <div className="space-y-6">
      <div className="cg-card p-6 flex flex-col items-center justify-center text-center">
        ...
        <CgButton 
          ...
          isLoading={generateMutation.isPending || scheme.is_generating}
        >
          <Play className="mr-2" />
          Generate Results
        </CgButton>
