...
export function SchoolOrgAdmin({ capabilities }: { capabilities: any }) {
  ...
  const studentColumns = [
    ...
    { key: "prn", header: profile?.terminology?.identifier || "Roll No" },
    { key: "batch", header: profile?.terminology?.program || "Standard" },
  ...
  return (
    <DashboardLayout role="ORG_ADMIN">
      <PageHeader 
        title={`${profile?.terminology?.institution || "School"} Dashboard`} 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        ...
        />
        <StatCard
          title={`Total ${profile?.terminology?.educator || "Teachers"}`} 
