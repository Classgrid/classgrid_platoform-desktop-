...
export function CollegeOrgAdmin({ capabilities }: { capabilities: any }) {
  ...
  const studentColumns = [
    ...
    { key: "prn", header: profile?.terminology?.identifier || "PRN" },
    { key: "branch", header: profile?.terminology?.program || "Branch" },
    { key: "batch", header: "Batch (Year)" },
    ...(capabilities.hasDivisions ? [{ key: "department", header: profile?.terminology?.group || "Division" }] : []),
  ...
  return (
    <DashboardLayout role="ORG_ADMIN">
      <PageHeader 
        title={`${profile?.terminology?.institution || "Engineering College"} Dashboard`} 
