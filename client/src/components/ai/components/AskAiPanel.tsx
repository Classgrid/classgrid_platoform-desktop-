      ...
      {blocks.map((block, index) => {
        ...
        if (block.type === "code") {
          ...
          const lineRows = highlightedLines.map((line, i) =>
            `<div class="flex w-max min-w-full"><span class="sticky left-0 z-10 shrink-0 w-14 pr-4 text-right select-none text-zinc-600 bg-[#111113] border-r border-white/5">${i + 1}</span><span class="px-4 whitespace-pre">${line || " "}</span></div>`
          ).join("");
