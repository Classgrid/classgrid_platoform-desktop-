...
type DataPoint = InputDataPoint & {
  name: string;
  color: string;
};

interface CgDonutChartProps {
  data: InputDataPoint[];
  height?: number;
  valuePrefix?: string;
  valueSuffix?: string;
...
export function CgDonutChart({
  ...
  const normalizedData: DataPoint[] = data?.map((entry, index) => ({
    ...entry,
    name: entry.name ?? entry.label ?? entry.id ?? `Item ${index + 1}`,
    color: entry.color ?? entry.fill ?? DEFAULT_COLORS[index % DEFAULT_COLORS.length] ?? "hsl(var(--primary))",
  }));
